...
class SiteplanEditor extends Editor {
  ...
  show() {
    ...
    if (!sp) {
      // No site-plan image (import skipped it): still show the building index so the
      // floors are reachable, plus a hint to add a site plan via re-import.
      this.app.setToolbar([]);
      stage.append(Dom.el('div', { class: 'siteplan-view' }, [
        Dom.el('div', { class: 'empty' },
          'No site plan image — pick a building from the index →'),
        this._legend(null),
      ]));
      return;
    }
    ...
    const img = Dom.el('img', { src: '/' + sp.image, alt: 'siteplan' });
  ...
  _legend(s) {
    ...
    const hover = (dir, on) => { if (!s) return; const n = s.querySelector(`[data-hs="${CSS.escape(dir)}"]`); if (n) n.classList.toggle('hot', on); };
