...
class FloorEditor extends Editor {
  ...
  show() {
    ...
    // A floor is one or more sheets (e.g. a floor drawn across two plans) tiled into
    // a grid; they share one normalized coordinate space spanning the whole canvas.
    // `floorLayout` is the single source
    ...
    const imgs = base.cells.map(c => Dom.el('img', { class: 'sheet', src: '/' + c.image, alt: f.label,
  ...
  /** Synced cache entry for a placement, or null if it's no longer in NetBox. */
  _cacheItem(p) {
  ...
  openRackPanel(room) {
    ...
    refreshBtn.onclick = async () => {
      ...
      try {
        const r = await this.netbox.syncRoom(room.location.id, room.label || room.location.name);
        await this.store.reloadRackCache();
        Toast.show('Refreshed ' + (room.label || room.location.name)
          + ' · ' + r.racks + ' racks · ' + r.devices + ' devices');
        this.render();             // restyle stale markers against the new cache
    ...
    if (!this.store.rackCache.locations[room.location.id])
      body.append(Dom.el('div', { class: 'hint' },
        'No rack data for this room yet — click Refresh racks to pull it from NetBox.'));
